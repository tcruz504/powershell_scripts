get-adcomputer -filter {Enabled -eq $true} -server ****** -Properties CanonicalName | foreach-object{
    ...
    $_ | Select-Object Name, CanonicalName, @{Name="MS017-010 Installed?";Expression={$installed}} | Export-Csv -Path C:\Users\"Destination"\Desktop\Patches\MS017-010_Summary.csv -Append -NoTypeInformation;
    
    #Output specific installed patch information to another file
    $patches | Export-Csv -Path C:\Users\"destination"\Desktop\Patches\MS017-010.csv -Append -NoTypeInformation
